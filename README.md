...
The Situs method implemented here follows the real-space convolution approach established in the Situs package [Wriggers2010, Wriggers1999]. This implementation is based on the `pdb2vol` tool from Situs.

**Important: B-factors are completely ignored in this method.** The map is generated by:

1. **Projection to Lattice**: Atoms are projected onto a cubic lattice using **trilinear interpolation**, with weights determined either by atomic mass (`--situs-mass`) or unit weight (`--situs-no-mass`). During projection, the lattice smoothing variance is calculated for optional correction later.

2. **Kernel Convolution**: The lattice is then convolved with the selected 3D kernel, where the kernel width is determined solely by the target resolution. No thermal motion parameters affect the calculation.

#### Kernel Functions

The method offers five distinct kernel types from the original Situs implementation, each with a different mathematical form. The kernels are defined as functions of the distance $r$ from the kernel center:

| Kernel Type | Function $K(r)$ | Formula |
|-------------|---------------|---------|
| **Gaussian** | $\exp\left(-\dfrac{1.5 r^2}{\sigma^2}\right)$ | where $\sigma$ is the Gaussian width |
| **Triangular** | $\max\left(0, 1 - \dfrac{0.5r}{r_h}\right)$ | $r_h$ is the half-max radius |
| **Semi-Epanechnikov** | $\max\left(0, 1 - \dfrac{0.5 r^{1.5}}{r_h^{1.5}}\right)$ | $r_h$ is the half-max radius |
| **Epanechnikov** | $\max\left(0, 1 - \dfrac{0.5 r^2}{r_h^2}\right)$ | $r_h$ is the half-max radius |
| **Hard Sphere** | $\max\left(0, 1 - \dfrac{0.5 r^{60}}{r_h^{60}}\right)$ | $r_h$ is the half-max radius |
...
Each kernel type has a specific cutoff radius beyond which the kernel value is zero:

| Kernel Type | Cutoff Radius |
|-------------|---------------|
| Gaussian | $r_c = 3\sigma$ |
| Triangular | $r_c = 2r_h$ |
| Semi-Epanechnikov | $r_c = 2^{1/1.5} r_h$ |
| Epanechnikov | $r_c = \sqrt{2} r_h$ |
| Hard Sphere | $r_c = 2^{1/60} r_h$ |

#### Lattice Smoothing Correction

The initial projection onto a lattice using trilinear interpolation introduces an inherent amount of blurring. An optional correction can be applied that accounts for this lattice smoothing:
...
| `--situs-kernel-type` | string | `gaussian` | Kernel type by name: `gaussian`, `triangular`, `semi-epanechnikov`, `epanechnikov`, `hard-sphere` |
